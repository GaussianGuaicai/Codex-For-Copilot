...
- Keep one user-visible `#webSearch` tool. Codex models intercept it into the hosted `web_search` tool, so its `{ query }` schema must never enter Codex function tools or Native Tool Search. Other models execute it through `hostedTools/webSearchExecutor.ts`, which runs one isolated streamed Responses request and must not join conversation continuation, branch reuse, or Native Tool Search. The Codex backend rejects non-streaming requests, so the fallback must go through `streamResponseText` rather than a one-shot `responses.create` call.
